...
# Fit Training Set to Logistic Regression Model
...
# Evaluate model, Predicted vs. Actual (Confusion Matrix)
...
X1, X2 = np.meshgrid(np.arange(start = X_set[:, 0].min() - 1, stop = X_set[:,0].max() + 1, step = 0.01), np.arange(X_set[:,1].min() - 1, stop = X_set[:,1].max() + 1, step = 0.01))
